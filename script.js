...
let candidateName = '';
const BRAND_NAME = 'NEET Tests by Hustler';
const BRAND_BADGE = 'NT';

function badgeFor(name){
  return escapeHtml((name || '').trim().slice(0,2).toUpperCase() || BRAND_BADGE);
}

/* Institutes can optionally ship a logo image at assets/logos/<slug>.png,
   where <slug> is the institute name lowercased with everything except
   letters/numbers stripped (e.g. "PW" -> pw.png, "Aakash" -> aakash.png,
   "Allen Digital" -> allendigital.png). If no matching file exists (or the
   institute has no logo yet) the badge quietly falls back to a two-letter
   initials chip — nothing breaks if a logo is missing. */
function logoSlugFor(name){
  return (name || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function brandBadgeHTML(name){
  const label = badgeFor(name);
  const slug = logoSlugFor(name);
  if(!slug) return label;
  return `<img src="assets/logos/${slug}.png" alt="" class="badge-logo" onerror="handleBadgeImgError(this)"><span class="badge-fallback">${label}</span>`;
}
...
function setBadge(elId, name){
  const el = document.getElementById(elId);
  if(!el) return;
  if(name) el.innerHTML = brandBadgeHTML(name);
  else el.textContent = BRAND_BADGE;
}
...
function parseQuestions(text){
  ...
  for(const block of blocks){
    ...
    const options = {};
    let answer = null;
    const images = []; // question-level images — any number of "Image:" lines
    ...
    for(const rawLine of block){
      ...
      if(imgMatch){ images.push(imgMatch[1].trim()); continue; }

      const optMatch = rawLine.match(optionRe);
      if(optMatch){
        const letter = optMatch[1].toUpperCase();
        const val = optMatch[2].trim();
        // An option can itself be an image, e.g. "B) Image: <url-or-data-uri>"
        const optImgMatch = val.match(imageRe);
        options[letter] = optImgMatch ? { image: optImgMatch[1].trim() } : { text: val };
        continue;
      }
    ...
    if(optCount >= 2 && answer && options[answer] !== undefined){
      qid++;
      parsed.push({ id: qid, text: qText, options, answer, images });
    }
  }
  return parsed;
}
...
let selectedSeries = null;    // which test series (within the selected institute) the visitor is currently browsing
...
function renderInstituteList(){
  ...
  institutes.forEach(inst => {
    ...
    const instSeriesCount = Object.keys(LIB[inst]).length;
    const card = document.createElement('div');
    card.className = 'institute-card';
    card.setAttribute('role', 'button');
    card.setAttribute('tabindex', '0');
    card.setAttribute('aria-label', 'Open ' + inst);
    card.innerHTML = `
      <span class="brand-badge">${brandBadgeHTML(inst)}</span>
      <div>
        <p class="institute-card-name">${escapeHtml(inst)}</p>
        <div class="institute-card-count"><b>${instSeriesCount}</b> series &middot; <b>${instTestCount}</b> test${instTestCount===1?'':'s'}</div>
      </div>
      <span class="institute-card-arrow">View series &rarr;</span>
    `;
    const go = () => selectInstitute(inst);
...
function selectInstitute(inst){
  ...
  document.getElementById('screen-series').classList.add('active');
  document.getElementById('selectedInstituteName').textContent = inst;
  setBadge('seriesScreenBadge', inst);
  renderSeriesForInstitute(inst);
}

function backToInstitutes(){
  selectedInstitute = null;
  selectedSeries = null;
  document.getElementById('screen-series').classList.remove('active');
...
/* ---------- Screen: series within the selected institute ---------- */
function renderSeriesForInstitute(inst){
  const LIB = computeLibrary();
  ...
  if(seriesNames.length === 0){
    container.innerHTML = '<div class="test-empty">No test series are available for this institute yet.</div>';
    return;
  }

  seriesNames.forEach(series => {
    const testCount = seriesMap[series].length;
    const card = document.createElement('div');
    card.className = 'institute-card';
    card.setAttribute('role', 'button');
    card.setAttribute('tabindex', '0');
    card.setAttribute('aria-label', 'Open ' + series);
    card.innerHTML = `
      <span class="brand-badge">${badgeFor(series)}</span>
      <div>
        <p class="institute-card-name">${escapeHtml(series)}</p>
        <div class="institute-card-count"><b>${testCount}</b> test${testCount===1?'':'s'} available</div>
      </div>
      <span class="institute-card-arrow">View tests &rarr;</span>
    `;
    const go = () => selectSeries(inst, series);
    card.addEventListener('click', go);
    card.addEventListener('keydown', (e) => { if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); go(); } });
    container.appendChild(card);
  });
...
function selectSeries(inst, series){
  selectedSeries = series;
  document.getElementById('screen-series').classList.remove('active');
  document.getElementById('screen-tests').classList.add('active');
  setBadge('testsScreenBadge', inst);
  document.getElementById('testsScreenInstitute').textContent = inst;
  document.getElementById('selectedSeriesName').textContent = series;
  renderTestsForSeries(inst, series);
}
...
/* ---------- Screen: tests within the selected series ---------- */
function renderTestsForSeries(inst, series){
  const LIB = computeLibrary();
  const container = document.getElementById('libraryGroups');
  container.innerHTML = '';

  const tests = (LIB[inst] && LIB[inst][series]) || [];

  if(tests.length === 0){
    container.innerHTML = '<div class="test-empty">No tests are available for this series yet.</div>';
    return;
  }
...
function startQuiz(){
  ...
  setBadge('quizBrandBadge', selectedInstitute);
...
function renderQuestion(){
  ...
  document.getElementById('qText').innerHTML = renderRichText(q.text);

  const qImgWrap = document.getElementById('qImages');
  qImgWrap.innerHTML = '';
  ...
  Object.keys(q.options).sort().forEach(letter => {
    ...
    div.className = 'option' + (selected ? ' selected' : '') + (opt.image ? ' option-image' : '');
    const contentHtml = opt.image
      ? `<img class="opt-image" src="${opt.image}" alt="Option ${letter}">`
      : `<label for="opt-${letter}">${renderInlineRich(opt.text)}</label>`;
    div.innerHTML = `
...
function submitTest(autoSubmitted){
  ...
  setBadge('resultBrandBadge', selectedInstitute);
  document.getElementById('resultInstituteName').textContent = selectedInstitute || BRAND_NAME;
  const now = new Date();
  ...
  reviewData.forEach((item, i) => {
    ...
    Object.keys(q.options).sort().forEach(letter => {
      ...
      const content = opt.image
        ? `<img class="review-opt-image" src="${opt.image}" alt="Option ${letter}">`
        : renderInlineRich(opt.text);
      optsHtml += `<div class="review-opt ${cls}">${letter}. ${content}</div>`;
    });
    const imagesHtml = (q.images && q.images.length)
      ? `<div class="review-images">${q.images.map(src => `<img class="review-q-image" src="${src}" alt="Question figure">`).join('')}</div>`
      : '';
    ...
    div.innerHTML = `
      ...
      <div class="rq">${renderRichText(q.text)}</div>
      ${imagesHtml}
...
/* Escapes a line of text, but first pulls out \frac{num}{den} tokens and
   renders them as a proper stacked fraction (numerator over denominator,
   divider line). Everything else is escaped normally — this is the only
   markup recognised inside question/option text besides the table syntax
   in renderRichText. Example source: "P = \frac{2x}{n+1}" */
function renderInlineRich(str){
  if(str == null) return '';
  const s = String(str);
  ...
  while((m = FRAC_RE.exec(s)) !== null){
    ...
    const num = escapeHtml(m[1]);
    const den = escapeHtml(m[2]);
    result += `<span class="frac"><span class="frac-num">${num}</span><span class="frac-den">${den}</span></span>`;
...
/* Renders question/statement text as safe HTML, turning any block of
   markdown-style pipe rows ( | cell | cell | ) into a real <table>,
   e.g. for "Match the Following" questions. Everything else is escaped
   (via renderInlineRich, which also expands \frac{}{} into a stacked
   fraction) and line breaks are preserved. No raw HTML from the source
   file is ever trusted — every table cell and text line goes through
   escaping. */
function renderRichText(raw){
  ...
  const flushText = () => {
    if(textBuf.length){
      const escaped = textBuf.map(l => renderInlineRich(l)).join('<br>');
      htmlParts.push(`<div class="q-text-block">${escaped}</div>`);
      textBuf = [];
    }
  };
  const flushTable = () => {
    if(tableBuf.length){
      const rows = tableBuf
        ...
        .filter(cells => !cells.every(c => /^:?-{2,}:?$/.test(c))); // drop markdown "|---|---|" separator rows
      if(rows.length){
        let html = '<table class="q-table">';
        rows.forEach((cells, i) => {
          const tag = i === 0 ? 'th' : 'td';
          html += '<tr>' + cells.map(c => `<${tag}>${renderInlineRich(c)}</${tag}>`).join('') + '</tr>';
        });
        html += '</table>';
        htmlParts.push(html);
      }
